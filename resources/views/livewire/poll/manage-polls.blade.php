<div>
    ...
    <div class="space-y-6">
        @forelse($polls as $poll)
            <div
                class="bg-white shadow-xl rounded-xl p-6 transition duration-300 hover:shadow-2xl border border-gray-100">
                <div class="flex justify-between items-start">
                    <div class="flex-1 pr-4">
                        <h3 class="text-2xl font-bold text-gray-900 mb-1">{{ $poll->title }}</h3>
                        <p class="mt-1 text-gray-500 italic">{{ $poll->description }}</p>

                        {{-- Options List (Enhanced with a subtle border) --}}
                        <div class="mt-5 pt-3 border-t border-gray-100">
                            <h4 class="text-xs font-semibold uppercase text-indigo-600 mb-2 tracking-wider">Pilihan:
                            </h4>
                        ...
                        <div class="mt-5">
                            <span @class([
                                ...
                                'bg-green-100 text-green-700 border border-green-300' => $poll->isOpen(),
                                'bg-red-100 text-red-700 border border-red-300' => !$poll->isOpen(),
                            ])>
        ...
        @empty
            <div class="text-center py-16 bg-white rounded-xl shadow-xl border border-gray-100">
                <svg class="w-14 h-14 mx-auto text-gray-400 mb-4" fill="none" stroke="currentColor"
                    viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01">
                    </path>
                </svg>
                <h3 class="mt-2 text-lg font-semibold text-gray-900">Belum ada jajak pendapat</h3>
                <p class="mt-1 text-sm text-gray-500">Mulai dengan membuat jajak pendapat pertama Anda.</p>
                <div class="mt-8">
                    <button wire:click="openCreateModal"
                        class="inline-flex items-center px-6 py-3 bg-indigo-600 border border-transparent rounded-full font-extrabold text-sm text-white uppercase tracking-wider shadow-lg hover:bg-indigo-700 focus:outline-none focus:ring-4 focus:ring-indigo-300 transition ease-in-out duration-300 transform hover:scale-[1.02]">
                        <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="fixed inset-0 z-10 w-screen overflow-y-auto">
            <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                <div x-show="$wire.showCreateModal" x-transition:enter="ease-out duration-300"
                    ...
                    class="relative transform overflow-hidden rounded-xl bg-white px-6 pb-6 pt-7 text-left shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:p-8">
                    ...
                    <div class="mt-6">
                        {{-- Title Field --}}
                        <div class="mb-5">
                            <label for="title" class="block text-sm font-semibold text-gray-700 mb-1">Judul Jajak
                                Pendapat</label>
                        ...
                        <div class="mb-5">
                            ...
                            <textarea wire:model="description" id="description" rows="3"
                                ...
                                class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-3"></textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        {{-- Options Fields --}}
                        <div class="mb-5">
                            <label class="block text-sm font-semibold text-gray-700 mb-2">Pilihan</label>
                            <div class="space-y-3">
                                @foreach ($options as $index => $option)
                                    <div class="flex items-center space-x-3">
                                        <input wire:model="options.{{ $index }}" type="text"
                                            placeholder="Pilihan {{ $index + 1 }}"
                                            class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-3">
                                        @if (count($options) > 2)
                            ...
                            <button wire:click="addOption" type="button"
                                class="mt-3 inline-flex items-center text-sm font-medium text-indigo-600 hover:text-indigo-500 bg-indigo-50 px-3 py-1.5 rounded-full transition">
                    ...
                    {{-- Modal footer --}}
                    <div class="mt-6 pt-4 border-t border-gray-100 sm:flex sm:flex-row-reverse">
                        <button wire:click="{{ isset($selectedPoll) ? 'update' : 'create' }}" type="button"
                            class="inline-flex w-full justify-center rounded-lg bg-indigo-600 px-4 py-2 text-base font-semibold text-white shadow-md hover:bg-indigo-700 transition sm:ml-3 sm:w-auto">
                            {{ isset($selectedPoll) ? 'Simpan Perubahan' : 'Buat Sekarang' }}
                        </button>
                        <button wire:click="$set('showCreateModal', false)" type="button"
                            class="mt-3 inline-flex w-full justify-center rounded-lg bg-white px-4 py-2 text-base font-semibold text-gray-700 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50 transition sm:mt-0 sm:w-auto">
                            Batal
                        </button>
                    </div>
                </div>
    ...
    <style>
        ...
        .aksi-button {
            @apply p-2 rounded-full border border-gray-200 text-gray-400 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition duration-150 ease-in-out;
        }
    </style>
</div>
