<div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-4 mb-4">
        <div class="flex justify-between items-center">
            ...
            @else
                <span class="text-sm text-gray-500 dark:text-gray-400 italic">Project is completed</span>
            @endif
        </div>
    </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-4 {{ $task->is_completed ? 'border-2 border-green-500' : '' }}">
                <div class="flex justify-between items-start mb-3">
                    ...
                    <div class="flex space-x-2 ml-4">
                        @if(!$task->is_completed)
                            ...
                            @endif
                        @else
                            <span class="text-xs text-gray-500 dark:text-gray-400 italic">Completed tasks cannot be edited</span>
                        @endif
                    </div>
                </div>
                ...
                @else
                    <div class="mt-3 border-t border-gray-200 dark:border-gray-700 pt-3">
                        <p class="text-sm text-gray-500 dark:text-gray-400 italic text-center mb-3">
                                ...
                                <div class="space-y-2">
                                    @foreach($task->entries()->latest()->get() as $entry)
                                        <div class="flex justify-between items-center text-xs text-gray-600 dark:text-gray-400">
                                            <span>{{ $entry->started_at->format('M d, Y H:i') }} - {{ $entry->stopped_at ? $entry->stopped_at->format('H:i') : 'Running' }}</span>
