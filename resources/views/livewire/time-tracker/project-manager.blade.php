<div>
    ...
    <div class="bg-gradient-to-r from-blue-500 to-blue-600 dark:from-blue-600 dark:to-blue-700 rounded-xl shadow-lg p-6 mb-6">
        <div class="flex justify-between items-center">
            <div>
                <h2 class="text-2xl font-bold text-white mb-1">My Projects</h2>
                <p class="text-blue-100 text-sm">Select a project to manage tasks and track time</p>
            </div>
    ...
    @if(session()->has('success'))
        <div class="bg-green-50 border-l-4 border-green-500 text-green-700 px-4 py-3 rounded-lg relative mb-4 shadow-sm" role="alert">
            <div class="flex items-center">
                <svg class="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 20 20">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-5 mb-6">
        ...
        <div class="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
            <p class="text-sm text-gray-600 dark:text-gray-400">
                <span class="font-semibold text-gray-900 dark:text-gray-100">{{ $projects->count() }}</span> 
                project{{ $projects->count() !== 1 ? 's' : '' }} found
                @if($search)
                    <span class="ml-1">matching "<span class="font-medium text-blue-600 dark:text-blue-400">{{ $search }}</span>"</span>
                @endif
            </p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        @forelse($projects as $project)
            <div wire:click="selectProject({{ $project->id }})" 
                 class="group relative bg-white dark:bg-gray-800 rounded-xl shadow-md hover:shadow-xl transition-all duration-300 cursor-pointer overflow-hidden {{ $project->is_completed ? 'ring-2 ring-green-400' : 'hover:scale-105' }}">
                
                <!-- Colored top border -->
                <div class="h-1.5 bg-gradient-to-r from-blue-400 to-purple-500 {{ $project->is_completed ? 'from-green-400 to-emerald-500' : '' }}"></div>
                    ...
                    <div class="flex justify-between items-start mb-3">
                        <div class="flex items-start flex-1">
                            ...
                            <div class="flex-1 min-w-0">
                                <h3 class="font-bold text-lg text-gray-900 dark:text-gray-100 mb-1 truncate {{ $project->is_completed ? 'line-through text-gray-500' : '' }}">
                                    {{ $project->title }}
                                </h3>
                                @if($project->is_completed)
                                    <span class="inline-flex items-center px-2.5 py-0.5 text-xs font-semibold text-green-800 bg-green-100 rounded-full dark:bg-green-900 dark:text-green-200 mb-2">
                        ...
                        <!-- Action Buttons -->
                        <div class="flex space-x-1 ml-3 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
        ...
        @empty
            <div class="col-span-full bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-900 rounded-xl shadow-md p-12 text-center">
                <div class="max-w-sm mx-auto">
                    ...
                    <p class="text-gray-600 dark:text-gray-400 mb-4">Start organizing your work by creating your first project.</p>
                    <button wire:click="openCreateModal" 
    ...
    @if($showModal)
        <div class="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm overflow-y-auto h-full w-full z-50 flex items-center justify-center p-4" 
            ...
            <div class="relative w-full max-w-lg bg-white dark:bg-gray-800 rounded-2xl shadow-2xl transform transition-all" 
                ...
                <!-- Modal Header -->
                <div class="bg-gradient-to-r from-blue-500 to-blue-600 dark:from-blue-600 dark:to-blue-700 px-6 py-4 rounded-t-2xl">
