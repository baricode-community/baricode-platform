...
<div class="">
    <div class="mb-8">
        <div class="flex items-center mb-4">
            {{-- Tombol Kembali --}}
            <a href="{{ route('satu-tapak.show', $habit) }}" wire:navigate
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Total Hari</p>
                    <p class="text-2xl font-semibold text-gray-900 dark:text-gray-100">{{ $stats['total_days'] }}</p>
                </div>
            </div>
        </div>
            ...
            <div class="flex items-center">
                <div class="flex-shrink-0">
                    <div class="w-8 h-8 bg-green-100 dark:bg-green-900/50 rounded-full flex items-center justify-center">
                        <span class="text-green-600 dark:text-green-300 text-lg">⏱️</span>
                    </div>
                </div>
                <div class="ml-4">
                    <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Hari Berlalu</p>
                    <p class="text-2xl font-semibold text-gray-900 dark:text-gray-100">{{ $stats['elapsed_days'] }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Sisa Hari</p>
                    <p class="text-2xl font-semibold text-gray-900 dark:text-gray-100">{{ $stats['remaining_days'] }}</p>
                </div>
            </div>
        </div>
            ...
            <div class="flex items-center">
                <div class="flex-shrink-0">
                    <div class="w-8 h-8 bg-purple-100 dark:bg-purple-900/50 rounded-full flex items-center justify-center">
                        <span class="text-purple-600 dark:text-purple-300 text-lg">👥</span>
                    </div>
                </div>
                <div class="ml-4">
                    <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Peserta</p>
                    <p class="text-2xl font-semibold text-gray-900 dark:text-gray-100">{{ $habit->approvedParticipants->count() }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
            <h2 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-6">Progress Keseluruhan</h2>
            ...
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-3 gap-4">
                    ...
                    <div class="text-center p-4 bg-green-50 dark:bg-green-900/50 rounded-lg">
                        <div class="text-2xl font-bold text-green-600 dark:text-green-300">{{ $stats['present_logs'] }}</div>
                        <div class="text-sm text-gray-600 dark:text-gray-400">Hadir</div>
                        <div class="text-xs text-green-600 dark:text-green-300">{{ $presentPercentage }}%</div>
                    </div>
                    
                    <div class="text-center p-4 bg-yellow-50 dark:bg-yellow-900/50 rounded-lg">
                        <div class="text-2xl font-bold text-yellow-600 dark:text-yellow-300">{{ $stats['late_logs'] }}</div>
                        <div class="text-sm text-gray-600 dark:text-gray-400">Terlambat</div>
                        <div class="text-xs text-yellow-600 dark:text-yellow-300">{{ $latePercentage }}%</div>
                    </div>
                    
                    <div class="text-center p-4 bg-red-50 dark:bg-red-900/50 rounded-lg">
                        <div class="text-2xl font-bold text-red-600 dark:text-red-300">{{ $stats['absent_logs'] }}</div>
                        <div class="text-sm text-gray-600 dark:text-gray-400">Tidak Hadir</div>
                        <div class="text-xs text-red-600 dark:text-red-300">{{ $absentPercentage }}%</div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
            <h2 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-6">Progress per Peserta</h2>
                    ...
                    <div class="p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                        <div class="flex items-center justify-between mb-2">
                            <div class="flex items-center space-x-2">
                                ...
                                <span class="font-medium text-gray-900 dark:text-gray-100">{{ $participant->user->name }}</span>
                                @if($participant->user_id === $habit->user_id)
                                    <span class="bg-blue-100 text-blue-800 text-xs font-medium px-2 py-1 rounded-full dark:bg-blue-900/50 dark:text-blue-300">Creator</span>
                                @endif
                            </div>
                            <span class="text-sm font-medium text-gray-600 dark:text-gray-300">{{ $userPercentage }}%</span>
                        </div>
    ...
    <div class="mt-8 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
        <h2 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-6">Aktivitas Terbaru</h2>
        
        @if($habit->logs->isEmpty())
            <div class="text-center py-8">
                <p class="text-gray-500 dark:text-gray-400">Belum ada aktivitas yang dicatat</p>
            </div>
        @else
            <div class="space-y-3">
                    ...
                    <div class="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                        <div class="flex items-center space-x-3">
                            ...
                            <div>
                                ...
                                <div class="text-sm text-gray-500 dark:text-gray-400">{{ $log->log_date->format('d M Y') }} - {{ $log->log_time->format('H:i') }}</div>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center space-x-3">
                            ...
                            @elseif($log->status === 'late')
                                <span class="bg-yellow-100 text-yellow-800 text-sm font-medium px-3 py-1 rounded-full dark:bg-yellow-900/50 dark:text-yellow-300">
                                    ⏰ Terlambat
                                </span>
                            @else
                            ...
                            @if($log->notes)
                                <div class="relative group">
                                    <div class="text-gray-400 dark:text-gray-500 cursor-help">💬</div>
                                    <div class="absolute bottom-full right-0 mb-2 hidden group-hover:block bg-gray-800 text-white text-sm rounded-lg p-2 whitespace-nowrap max-w-xs z-10">
                                        {{ $log->notes }}
                                    </div>
                                </div>
                            @endif
