<x-layouts.app :title="__('Course Details')">
    ...
    <div class="mb-10 flex flex-col p-2 md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div class="flex items-center gap-4">
            <span
                class="inline-flex items-center px-3 py-1 rounded-full bg-green-100 dark:bg-green-900 text-green-700 dark:text-green-300 text-sm font-semibold">
    ...
    <div class="bg-white/80 dark:bg-gray-800/80 shadow-xl rounded-2xl p-8">
        ...
        <div x-data="{ tab: '{{ $tab }}' }">
            ...
            <div class="md:hidden flex mb-6">
                <button @click="tab = 'info'"
                    :class="tab === 'info' ? 'bg-blue-600 text-white' :
                        'bg-gray-200 text-gray-700 dark:bg-gray-700 dark:text-gray-300'"
                    class="flex-1 py-2 rounded-l-lg font-semibold transition">
                    Informasi Kursus
                </button>
                <button @click="tab = 'status'"
                    :class="tab === 'status' ? 'bg-blue-600 text-white' :
                        'bg-gray-200 text-gray-700 dark:bg-gray-700 dark:text-gray-300'"
                    class="flex-1 py-2 rounded-r-lg font-semibold transition">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <!-- Informasi Kursus -->
                <div x-show="tab === 'info' || window.innerWidth >= 768" x-cloak
                    ...
                    @if (isset($enrollmentSessions) && count($enrollmentSessions))
                        <div class="mt-6">
                            ...
                            <ul
                                class="divide-y divide-gray-200 dark:divide-gray-700 bg-blue-50 dark:bg-gray-800 rounded-lg shadow-sm overflow-hidden">
                                @foreach ($enrollmentSessions as $session)
                                    <li
                                        class="py-4 px-5 flex flex-col md:flex-row md:items-center md:justify-between hover:bg-blue-100 dark:hover:bg-gray-700 transition-all duration-200">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <span class="text-gray-500 dark:text-gray-400 text-xs">
                                                (Sesi:
                                                <span
                                                    class="font-mono">{{ $session->reminder_1 ? \Carbon\Carbon::parse($session->reminder_1)->format('H:i') : '-' }}</span>,
                                                <span
                                                    class="font-mono">{{ $session->reminder_2 ? \Carbon\Carbon::parse($session->reminder_2)->format('H:i') : '-' }}</span>,
                                                <span
                                                    class="font-mono">{{ $session->reminder_3 ? \Carbon\Carbon::parse($session->reminder_3)->format('H:i') : '-' }}</span>)
                                            </span>
                                        </div>
                                    </li>
                                @endforeach
                ...
                <!-- Status Waktu Belajar -->
                <div x-show="tab === 'status' || window.innerWidth >= 768" x-cloak
                    class="flex flex-col justify-center items-center h-full bg-white dark:bg-gray-900 rounded-xl shadow-lg p-6">
                    @if ($enrollment->isWaktunyaBelajar())
                        <span
                            class="inline-flex items-center px-4 py-2 rounded-full bg-blue-200 dark:bg-blue-800 text-blue-800 dark:text-blue-200 text-base font-semibold shadow">
                    ...
                    @else
                        <div class="flex flex-col items-center">
                            <span
                                class="inline-flex items-center px-4 py-2 rounded-full bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 text-base font-semibold shadow">
                            ...
                            <div
                                class="mt-6 px-4 py-2 rounded bg-yellow-100 dark:bg-yellow-900 text-yellow-800 dark:text-yellow-200 text-sm font-semibold text-center">
        ...
        <!-- Alpine.js required for tab functionality -->
        ...
        <div class="mt-10">
            ...
            <div class="space-y-4">
                @foreach ($modules as $module)
                    <div
                        class="flex flex-col gap-2 p-5 bg-blue-50 dark:bg-gray-700 rounded-xl shadow hover:shadow-lg transition">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-2">
                                ...
                                @endif
                                <span
                                    class="font-semibold text-lg">{{ $module->module->name ?? 'Module Name Not Available' }}</span>
                            </div>
                        </div>
